...
pub fn run_all(pin_nums: &Vec<[String; 4]>, num_steps: u32, steps_per_sec: u32) {
	setup_gpios(pin_nums);
	go_n_steps(num_steps, pin_nums, find_sleep_time(steps_per_sec));
	takedown_gpios(pin_nums);
}

pub fn do_all<F>(pin_nums: &Vec<[String; 4]>, todo: F)
	where F : Fn(&str) {
...
pub fn set_all_at(pin_nums: &Vec<[String; 4]>, pin_i: usize, value: &str) {
	for pins in pin_nums {
    super::pins::gpio_cmd(&pins[pin_i], "value", &value);
  }
}

pub fn go_n_steps(n: u32, pin_nums: &Vec<[String; 4]>, sleep_time: Duration) {
