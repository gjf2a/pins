...
fn make_pins(mut pins: SplitWhitespace) -> [String; 4] {
	let mut result = [String::from("0"), String::from("0"), String::from("0"), String::from("0")];
	for i in 0..4 {
		result[i] = String::from(pins.next().unwrap());
	}
	return result;
}
